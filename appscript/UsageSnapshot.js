...
function getUsageWeekStart_(date) {
...
function extractPoolId_(value) {
  ...
  const m = s.match(/(MCPS-\d{4,})/i); // matches anywhere in the string
  return m ? m[1].toUpperCase() : s;
}

function getUsageHeaderMap_(sheet) {
...
function ensureHeaders_(sheet, headers) {
  ...
  if (lastCol === 0) {
    ...
    return getUsageHeaderMap_(sheet);
  }

  const hm = getUsageHeaderMap_(sheet);
...
function backfillMissingUsagePriced_(opts) {
  opts = opts || {};
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  ...
  const rawHeaders = raw.getRange(1, 1, 1, raw.getLastColumn()).getValues()[0]
    .map(function(h) { return String(h || "").trim(); });
  const rawMap = rawHeaders.reduce(function(acc, h, i) {
    if (h) acc[h] = i + 1;
    return acc;
  }, {});
  const tsCol = rawMap["Timestamp"];
  const poolCol = rawMap["pool_id"];
  if (!tsCol) throw new Error('Chemical_Usage_Log missing "Timestamp"');
  if (!poolCol) throw new Error('Chemical_Usage_Log missing "pool_id"');

  const since = opts.since ? new Date(opts.since) : new Date(Date.now() - 2 * 24 * 60 * 60 * 1000);
  const sinceTime = isNaN(since.getTime()) ? 0 : since.getTime();

  const existingKeys = {};
  if (priced.getLastRow() >= 2 && priced.getLastColumn() > 0) {
    const pricedHeaders = priced.getRange(1, 1, 1, priced.getLastColumn()).getValues()[0]
      .map(function(h) { return String(h || "").trim(); });
    const pricedMap = pricedHeaders.reduce(function(acc, h, i) {
      if (h) acc[h] = i + 1;
      return acc;
    }, {});
    const pricedValues = priced.getRange(2, 1, priced.getLastRow() - 1, pricedHeaders.length).getValues();
    pricedValues.forEach(row => {
      const pid = pricedMap["pool_id"] ? String(row[pricedMap["pool_id"] - 1] || "").trim() : "";
      const key = buildUsageDedupKey_(row, pricedHeaders, pid);
      if (key) existingKeys[key] = true;
    });
  }

  const result = { ok: true, since: since.toISOString(), scanned: 0, backfilled: 0, skipped: 0, errors: [] };
  if (raw.getLastRow() < 2) {
    Logger.log(JSON.stringify(result));
    return result;
  }

  const values = raw.getRange(2, 1, raw.getLastRow() - 1, rawHeaders.length).getValues();
  values.forEach((row, idx) => {
    const rowNum = idx + 2;
    const ts = new Date(row[tsCol - 1]);
    if (isNaN(ts.getTime()) || ts.getTime() < sinceTime) return;

    result.scanned++;
    const rawPool = String(row[poolCol - 1] || "").trim();
    const poolId = extractPoolId_(rawPool);
    const key = buildUsageDedupKey_(row, rawHeaders, poolId);
    if (existingKeys[key]) {
      result.skipped++;
      return;
    }
    ...
    try {
      // A failed prior attempt could have claimed the short-lived dedup key before
      // writing the priced row. Since no priced row exists, clear it for repair.
      PropertiesService.getScriptProperties().deleteProperty("dedup|usage_snapshot|" + key);
      snapshotUsageToPriced_({ range: raw.getRange(rowNum, 1) });
  ...
  if (result.errors.length) {
    result.errors.forEach(function(e) {
      Logger.log("Backfill row " + e.row + " failed: " + e.error);
    });
  }
  return result;
}

function backfillMissingUsagePriced(opts) {
  return backfillMissingUsagePriced_(opts);
}

function backfillMissingUsagePricedJune22() {
  return backfillMissingUsagePriced_({ since: "2026-06-22" });
}

function dryRunUsagePricedLatest() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const raw = ss.getSheetByName(RAW_SHEET);
  if (!raw) throw new Error(`Missing sheet: ${RAW_SHEET}`);
  return dryRunUsagePricedRow_(raw.getLastRow());
}

function dryRunUsagePricedJune22() {
  return dryRunMissingUsagePriced_({ since: "2026-06-22" });
}

function dryRunMissingUsagePriced_(opts) {
  opts = opts || {};
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const raw = ss.getSheetByName(RAW_SHEET);
  if (!raw) throw new Error(`Missing sheet: ${RAW_SHEET}`);

  const rawHm = getUsageHeaderMap_(raw);
  const tsCol = rawHm.map["Timestamp"];
  if (!tsCol) throw new Error('Chemical_Usage_Log missing "Timestamp"');

  const since = opts.since ? new Date(opts.since) : new Date(Date.now() - 2 * 24 * 60 * 60 * 1000);
  const sinceTime = isNaN(since.getTime()) ? 0 : since.getTime();
  const result = { ok: true, dryRun: true, since: since.toISOString(), scanned: 0, existing: 0, wouldBackfill: 0, errors: [] };
  ...
  const values = raw.getRange(2, 1, raw.getLastRow() - 1, rawHm.headers.length).getValues();
  values.forEach(function(row, idx) {
    ...
    result.scanned++;
    try {
      const rowResult = dryRunUsagePricedRow_(rowNum, true);
      if (rowResult.existsInUsagePriced) result.existing++;
      else result.wouldBackfill++;
    } catch (err) {
      result.errors.push({ row: rowNum, error: String(err) });
    }
  });
  ...
  if (result.errors.length) {
    result.errors.forEach(function(e) {
      Logger.log("Dry-run row " + e.row + " failed: " + e.error);
    });
  }
  return result;
}

function dryRunUsagePricedRow_(rowNum, quiet) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const raw = ss.getSheetByName(RAW_SHEET);
  if (!raw) throw new Error(`Missing sheet: ${RAW_SHEET}`);
  const priced = ss.getSheetByName(PRICED_SHEET);
  if (!priced) throw new Error(`Missing sheet: ${PRICED_SHEET}`);

  rowNum = Number(rowNum || raw.getLastRow());
  if (!rowNum || rowNum < 2 || rowNum > raw.getLastRow()) throw new Error("Invalid raw row: " + rowNum);

  const rawHm = getUsageHeaderMap_(raw);
  const rawRow = raw.getRange(rowNum, 1, 1, rawHm.headers.length).getValues()[0];
  const priceMap = buildPriceMap_();
  const unitCostHeaders = [];
  rawHm.headers.forEach(function(h) {
    if (priceMap.hasOwnProperty(h)) unitCostHeaders.push(`${h} Unit Cost (Snapshot)`);
  });

  const finalHeaders = rawHm.headers.concat(unitCostHeaders, ["Week Start", "WeekKey", "Visit # in Week", "Total Visit Chem Cost (Snapshot)"]);
  const pricedHm = getUsageHeaderMap_(priced);
  const missingHeaders = finalHeaders.filter(function(h) { return !pricedHm.map[h]; });

  const tsRaw = rawHm.map["Timestamp"] ? rawRow[rawHm.map["Timestamp"] - 1] : "";
  const ts = new Date(tsRaw);
  if (isNaN(ts.getTime())) throw new Error("Invalid Timestamp on raw row " + rowNum);

  const rawPool = rawHm.map["pool_id"] ? String(rawRow[rawHm.map["pool_id"] - 1] || "").trim() : "";
  if (!rawPool) throw new Error("Missing pool_id on raw row " + rowNum);
  const poolId = extractPoolId_(rawPool);

  let total = 0;
  rawHm.headers.forEach(function(h, i) {
    if (!priceMap.hasOwnProperty(h)) return;
    const qty = Number(rawRow[i]);
    if (!isFinite(qty) || qty === 0) return;
    total += qty * priceMap[h];
  });

  const dedupKey = buildUsageDedupKey_(rawRow, rawHm.headers, poolId);
  let existsInUsagePriced = false;
  if (priced.getLastRow() >= 2 && priced.getLastColumn() > 0) {
    const pricedHeaders = pricedHm.headers;
    const pricedValues = priced.getRange(2, 1, priced.getLastRow() - 1, pricedHeaders.length).getValues();
    existsInUsagePriced = pricedValues.some(function(row) {
      const pid = pricedHm.map["pool_id"] ? String(row[pricedHm.map["pool_id"] - 1] || "").trim() : "";
      return buildUsageDedupKey_(row, pricedHeaders, pid) === dedupKey;
    });
  }
  ...
  const result = {
    ...
    pool_id: poolId,
    totalChemCost: total,
    existsInUsagePriced: existsInUsagePriced,
    wouldAppend: !existsInUsagePriced,
    missingHeaders: missingHeaders,
    dedupKeyLength: dedupKey.length
  };
  if (!quiet) Logger.log(JSON.stringify(result));
  return result;
}
...
function snapshotUsageToPriced_(e) {
  ...
  if (poolId && priced.getLastRow() >= 2) {
    ...
    values.forEach(r => {
      ...
      const existingWeekStart = getUsageWeekStart_(t);
  ...
  try {
    if (poolId && poolId !== "OTHER / POOL NOT LISTED" && typeof checkStartupLifecycle === "function") {
