...
function handleUpdateLead_(payload) {
  ...
  // Billing fields — soft write (no error if column doesn't exist yet)
  const softSet = (colName, val) => {
    const idx = headers.map(h => String(h).toLowerCase().trim()).indexOf(colName.toLowerCase().trim());
    if (idx !== -1) sheet.getRange(rowIdx, idx + 1).setValue(val !== undefined && val !== null ? val : '');
  };
